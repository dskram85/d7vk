...
namespace dxvk {
  ...
  inline void DDraw4Surface::DownloadSurfaceData(DDraw4Surface* surface) {
    ...
    if (unlikely(surface->GetCommonSurface()->IsD3D9BackBuffer())) {
      if (d3dOptions->backBufferWriteBack || d3dOptions->forceLegacyPresent || d3dOptions->apitraceMode) {
        ...
        if (surface->IsInitialized() && surface->GetCommonSurface()->IsD3D9SurfaceDirty()) {
          BlitToDDrawSurface<IDirectDrawSurface4, DDSURFACEDESC2>(surface->GetShadowOrProxied(), surface->GetD3D9());
        ...
        if (!std::exchange(s_swapchainWarningShown, true))
          Logger::warn("DDraw4Surface::DownloadSurfaceData: Surface is a bound swapchain surface");
      }
      ...
      if (d3dOptions->depthWriteBack || d3dOptions->apitraceMode) {
        ...
        if (surface->IsInitialized() && surface->GetCommonSurface()->IsD3D9SurfaceDirty()) {
          BlitToDDrawSurface<IDirectDrawSurface4, DDSURFACEDESC2>(surface->GetProxied(), surface->GetD3D9());
          surface->GetCommonSurface()->UnDirtyD3D9Surface();
        }
      } else {
        ...
        if (!std::exchange(s_depthStencilWarningShown, true))
          Logger::warn("DDraw4Surface::DownloadSurfaceData: Surface is a bound depth stencil");
      }
    }
  }
