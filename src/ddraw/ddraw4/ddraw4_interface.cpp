...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE DDraw4Interface::QueryInterface(REFIID riid, void** ppvObject) {
    ...
    if (riid == __uuidof(IDirect3D3)) {
      ...
      if (unlikely(m_d3d6Intf == nullptr)) {
        ...
        m_d3d6Intf = new D3D6Interface(nullptr, std::move(ppvProxyObject), this);
      }
    ...
    if (unlikely(riid == __uuidof(IDirect3D))) {
      ...
      Com<D3D3Interface> d3d3Intf = new D3D3Interface(nullptr, std::move(ppvProxyObject), m_commonIntf->GetDDInterface());
    ...
    if (unlikely(riid == __uuidof(IDirect3D2))) {
      ...
      Com<D3D5Interface> d3d5Intf = new D3D5Interface(nullptr, std::move(ppvProxyObject), nullptr);
  ...
  HRESULT STDMETHODCALLTYPE DDraw4Interface::EnumSurfaces(DWORD dwFlags, LPDDSURFACEDESC2 lpDDSD, LPVOID lpContext, LPDDENUMSURFACESCALLBACK2 lpEnumSurfacesCallback) {
    ...
    while (surfaceIt != attachedSurfaces.end() && hr != DDENUMRET_CANCEL) {
      ...
      hr = lpEnumSurfacesCallback(ddraw4Surface.ref(), &surfaceIt->surface4Desc, lpContext);
  ...
  HRESULT STDMETHODCALLTYPE DDraw4Interface::GetCaps(LPDDCAPS lpDDDriverCaps, LPDDCAPS lpDDHELCaps) {
    ...
    if (likely(d3d9Device != nullptr)) {
      ...
      Logger::debug(str::format("DDraw4Interface::GetCaps: DDraw Total: ", total7));
      Logger::debug(str::format("DDraw4Interface::GetCaps: DDraw Free : ", free7));
